# FlexBE States and Behaviors for pick_and_place
...
This raw repository has several folders and files with the generic name `pick_and_place`.
...
and change the relevant `pick_and_place` text to `my_new_project_name` as needed.
...
## Example States in `pick_and_place_flexbe_states`
...
## Example Behaviors in `pick_and_place_flexbe_behaviors`
...
[`copy_behavior`](pick_and_place_flexbe_behaviors/bin/copy_behavior) script to copy a saved behavior 
&mdash; both the Python implementation and manifest `.xml` file &mdash; 
to the project source folder for long term storage.
Use `ros2 run pick_and_place_flexbe_behavior copy_behavior` to see the usage guide. 
